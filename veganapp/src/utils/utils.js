/*   */
import * as AllImages from '../media/realMedia/indexMedia.js';


const productos = [
    {
    texto : "Brownies",
    imgn :"https://www.recetasderechupete.com/wp-content/uploads/2019/11/Brownie-768x530.jpg",
    
    },
    {
    texto : "Pan de centeno",
    imgn :"https://www.rebanando.com/media/pan-de-centeno-y-espelta-integral_crop.jpg/rh/pan-integral.jpg",
    
    },
    {
    texto : "Rolls de canela y manzana",
    imgn :"https://cookpad.com/do/recipe/images/26329c2271a542f8",
    
    },
    {
    texto : "Variedad en budines",
    imgn :"https://cookpad.com/ar/recipe/images/a32310e20a99bfe6",
    
    },
];
const mock = [
    {
    texto : "<p>Ingredientes: <br>Harina integra<br> azucar mascabo ,<br>leche de coco , <br>cacao amargo ,<br> aceite de girasol ,<br>dulce de leche de coco.<p>",
    imgn : AllImages.alfajoresChocolate,
    id : 0,
    name : "<h4>Alfajor de chocolate <br> con dulce de leche de coco<h4>",
    },
    {
    texto : "<p>Ingredientes: <br>Harina integra<br> azucar mascabo ,<br>leche de coco ,<br> aceite de girasol ,<br>dulce de leche de coco.<p>",
    imgn :AllImages.alfajoresCoco,
    id : 1,
    name : "<h4>Alfajor integral <br> con dulce de leche de coco<h4>",
    },
    {
    texto : "<p>Ingredientes :<br> Harina integral organica ,<br> mix de semillas , sal y <br>aceite de girasol<p>.",
    imgn :AllImages.bizcochos,
    id : 3,
    name : "<h4>Bizcochos integrales <br> con mix de semillas<h4>",
    },
    {
    texto : "<p>Ingredientes:<br> Harina integral organica, <br> azucar mascabo, <br> leche de coco , <br> dulce de leche de coco ,<br> chocolate semiamargo , <br> aceite de girasol.<p> ",
    imgn :AllImages.medialunasChocolate,
    id : 4,
    name :"<h4>Medialunas rellenas <br> con crema bariloche<h4>" ,
    },
    {
    texto : "<p>Ingredientes:<br> Harina integral organica ,<br> azucar mascabo ,<br> jugo de naranja ,<br> ralladura de naranja ,<br> leche de avena ,<br> aceite de girasol ,<br> manzanas ,<br> y canela.<p>",
    imgn :AllImages.medialunasCoco,
    id : 5,
    name : "<h4>Medialunas rellenas <br> con crema bariloche<h4>",
    },
    {
    texto : "<p>Ingredientes :<br> Harina de centeno,<br>harina integral ,<br> sal,<br> agua y ,<br>aceite de girasol.<p>",
    imgn :AllImages.panCenteno,
    id : 6,
    name : "<h4>Pan de centeno <br>con mix de semillas<h4>",
    },
    {
    texto : "<p>Ingredientes :<br> Harina integral oganica ,<br> agua ,<br> aceite de girasol ,<br>sal y <br> mix de semillas.<p>   ",
    imgn :AllImages.panIntegralSemillas,
    id : 7,
    name : "<h4>Pan integral ,<br> con mix de semillas<h4>",
    },
    {
    texto : "<p>Chocolate y coco<br>Arandanos y limon <br>Banana y chips de chocolate <br>Manzana y canela<br> Limon  ,jengibre y coco <br>Zanahorias , naranjas y nueces<p>",
    imgn :AllImages.budinChocolate,
    id : 8,
    name : "<h4>Variedad en budines<h4>",
    },
];
export  {
    mock,
    productos
};
